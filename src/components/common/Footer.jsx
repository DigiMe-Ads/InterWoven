...
export default function Footer() {
  ...
  return (
    <footer className="relative overflow-hidden">
      <img src="/images/home/footer.jpg" alt="" className="absolute inset-0 w-full h-full object-cover" />
      <div className="absolute inset-0" style={{ background: "linear-gradient(135deg, rgba(180,190,230,0.88) 0%, rgba(160,175,220,0.88) 40%, rgba(190,195,235,0.85) 100%)" }} />
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-6 pt-14 pb-6">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10 pb-10">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }} className="space-y-4">
            <img src="/logo.png" alt="Interwoven" className="h-28 w-auto object-contain" />
          ...
          {/* COL 2: Contact — from global */}
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: 0.1 }} className="space-y-3">
            <h4 className="text-base font-extrabold text-[#1E2A4A]">Contact</h4>
            <div className="space-y-2">
              <p className="text-sm text-[#2D3D6B]/80 leading-relaxed">{global.address}</p>
              <a href={`mailto:${global.email}`} className="block text-sm text-[#2D3D6B]/80 hover:text-[#1E2A4A] transition-colors">{global.email}</a>
              <a href={`tel:${global.phone}`}    className="block text-sm text-[#2D3D6B]/80 hover:text-[#1E2A4A] transition-colors">{global.phone}</a>
            </div>
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: 0.2 }} className="space-y-3">
            ...
            <ul className="space-y-2">
              {quickLinks.map(link => (
                <li key={link.label}>
                  <Link to={link.href} className="text-sm text-[#2D3D6B]/80 hover:text-[#1E2A4A] transition-colors">{link.label}</Link>
                </li>
              ))}
            </ul>
          </motion.div>
          ...
          {/* COL 4: Newsletter */}
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: 0.3 }} className="space-y-3">
            <h4 className="text-base font-extrabold text-[#1E2A4A]">Newsletter</h4>
            <p className="text-sm font-bold text-[#1E2A4A]">{content.newsletterLabel}</p>
            <form onSubmit={handleSubscribe} className="space-y-2 pt-1">
              <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="Your email here" required
              ...
              {subStatus === "success" && <p className="text-xs font-semibold text-green-700 bg-green-100 rounded-lg px-3 py-1.5">✓ Subscribed successfully!</p>}
              {subStatus === "error"   && <p className="text-xs font-semibold text-red-700 bg-red-100 rounded-lg px-3 py-1.5">Something went wrong. Try again.</p>}
              <motion.button type="submit" disabled={subStatus === "submitting"}
              ...
              >
                {subStatus === "submitting" ? (
                  <><div className="w-3.5 h-3.5 border-2 border-white border-t-transparent rounded-full animate-spin" />Subscribing...</>
                ) : "Subscribe now"}
              </motion.button>
            </form>
          </motion.div>
