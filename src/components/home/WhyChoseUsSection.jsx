...
const STAT_ICONS = [
  ...
  <svg className="w-8 h-8 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.8} d="M14 10h4.764a2 2 0 011.789 2.894l-3.5 7A2 2 0 0115.263 21h-4.017c-.163 0-.326-.02-.485-.06L7 20m7-10V5a2 2 0 00-2-2h-.095c-.5 0-.905.405-.905.905a3.61 3.61 0 01-.608 2.006L7 11v9m7-10h-2M7 20H5a2 2 0 01-2-2v-6a2 2 0 012-2h2.5" />
  </svg>,
  <svg className="w-8 h-8 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
const STAT_BGS = [
  "linear-gradient(160deg, #7B8FE4 0%, #5468C0 100%)",
  "linear-gradient(160deg, #6275D0 0%, #4055B0 100%)",
  "linear-gradient(160deg, #4A5BBF 0%, #2D3D96 100%)",
];
...
export default function WhyChooseUsSection() {
  ...
  const stats = [
    { icon: STAT_ICONS[0], value: content.stat1Val, label: content.stat1Label, bg: STAT_BGS[0] },
    { icon: STAT_ICONS[1], value: content.stat2Val, label: content.stat2Label, bg: STAT_BGS[1] },
    { icon: STAT_ICONS[2], value: content.stat3Val, label: content.stat3Label, bg: STAT_BGS[2] },
  ];

  return (
    <section
      className="relative overflow-hidden py-20 px-6"
      style={{ background: "linear-gradient(135deg, #3B67BFE5 0%, #FFFFFF 100%)" }}
    >
        ...
        >
          {stats.map((stat, i) => (
            <motion.div
              ...
              style={{ background: stat.bg, width: 120, height: 108 }}
            >
              {stat.icon}
              <p className="text-xl font-extrabold text-white leading-none">{stat.value}</p>
              <p className="text-xs text-white/80 text-center leading-tight">{stat.label}</p>
            </motion.div>
          ))}
        </motion.div>
