...
export default function ContactPageSection() {
  const { content }         = useContent("contactPage");
  ...
  // Built from global content so admin edits flow through
  const contactCards = [
    {
      icon: (
    ...
    },
    {
      ...
      title: "Need Support?",
      lines: [global.phone, global.email],
    },
  ];
  ...
  return (
    <>
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-5">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="flex flex-col gap-1.5">
                  ...
                  <input name="name" required value={form.name} onChange={handleChange} placeholder="Your name here"
                ...
                <div className="flex flex-col gap-1.5">
                  ...
                  <input name="email" type="email" required value={form.email} onChange={handleChange} placeholder="Your email here"
                    className="w-full bg-white rounded-xl px-4 py-3 text-sm text-gray-500 outline-none placeholder-gray-300 focus:ring-2 focus:ring-[#4A5DAA]/30 transition" />
                </div>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="flex flex-col gap-1.5">
                  ...
                  <input name="subject" value={form.subject} onChange={handleChange} placeholder="Your subject."
                    className="w-full bg-white rounded-xl px-4 py-3 text-sm text-gray-500 outline-none placeholder-gray-300 focus:ring-2 focus:ring-[#4A5DAA]/30 transition" />
                </div>
              </div>
              <div className="flex flex-col gap-1.5">
              ...
              {status === "success" && (
                <p className="text-sm font-semibold text-white bg-green-500/30 rounded-xl px-4 py-2 text-center">✓ Message sent! We'll get back to you soon.</p>
              )}
              {status === "error" && (
                <p className="text-sm font-semibold text-white bg-red-500/30 rounded-xl px-4 py-2 text-center">Something went wrong. Please try again.</p>
              )}
          ...
          >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {contactCards.map((card, i) => (
                ...
                >
                  <div className="w-12 h-12 rounded-full bg-[#2D3D7A] flex items-center justify-center">{card.icon}</div>
            ...
            {/* Working hours — from global content */}
            <motion.div
              initial={{ opacity: 0, y: 15 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
            ...
            >
              ...
              <div>
                ...
                <p className="text-sm text-gray-400 mt-0.5">{global.hours}</p>
              </div>
            </motion.div>
          </motion.div>
        </div>
      ...
      <div className="w-full" style={{ height: 420 }}>
        <iframe
          title="Office Location" width="100%" height="100%"
